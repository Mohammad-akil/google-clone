...
const SearchResultHeader = () => {
  ...
  const clickHandler = (menu) => {
    let isTypedImage = menu.name === "Images";
    setSelectedMenu(menu.name);
    setImageSearch(isTypedImage ? true : false);
  };
  return (
    <div className="p-[15px] pb-0 md:pr-5 md:pl-20 md:pt-7 border-b border-[#ebebeb] flex md:block flex-col items-center sticky top-0 bg-white">
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center grow">
          ...
          <SearchInput from="search Result" />
        </div>
      ...
      <div className="flex ml-[-12px] mt-3">
        {menu.map((menu, index) => (
          <span
            key={index}
            className={`flex items-center p-3 text-[#5f6363] cursor-pointer relative ${
          ...
          >
            ...
            {selectedmenu === menu.name && (
              <span className="h-[3px] w-[calc(100%-20%)] absolute bg-[#1a73e8] bottom-0 left-[10px]" />
            )}
          </span>
        ))}
      </div>
